...
fn on_schedule_fails_without_controller_service() {
    ...
    assert_eq!(processor.on_schedule(&context), Err(MissingRequiredProperty("Dummy Controller Service")));
}
...
fn on_schedule_succeeds_with_any_controller_service() {
    ...
    context.properties.insert("Dummy Controller Service".to_string(), "nonexistent_controller_service".to_string());
