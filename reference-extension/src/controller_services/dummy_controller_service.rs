...
use minifi_native::{ControllerServiceContext, ControllerServiceDefinition, EnableControllerService, IdentifyComponent, Logger, MinifiError, Property};
...
impl EnableControllerService for DummyControllerService {
        ...
        Self: Sized
    {
       match context.get_property(&DATA)? {
           None => {
               Err(MissingRequiredProperty("Missing data"))
           }
           Some(data) => {
               Ok(Self{data})
           }
